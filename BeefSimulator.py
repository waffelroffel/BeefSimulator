...
class BeefSimulator:
    def __init__(self, dims, h, alpha, dt=None, conds=None):
        """
        dims: [ [x_start, x_len], [y_start, y_len], ... , [t_start, t_len] ]
        h: stepsize in each dim
        ...
        steps = [(s, s+l, int((l-s)/h) + 1) for s, l in dims[:-1]]
        ...
        self.h = h
        self.dt = dt if (dt is not None) else h**2/(4*alpha)
        print(f'{dt=}')
        ...
        self.t = np.linspace(t_start, t_end, self.t_steps)
    ...
    def apply_conditions(self, conds=None):
        ...
        if conds == None:
            conds = self.conds
        mesh = np.meshgrid(self.z, self.y, self.x)  # fix
        if self.D == 3:
            pass
        elif self.D == 2:
            print(self.U[..., 0].shape)
            self.U[0, ...] = conds[1](self.t)
            self.U[-1, :, :] = conds[2](self.t)
            self.U[:, 0, :] = conds[3](self.t)
            self.U[:, -1, :] = conds[4](self.t)
            ...
            if x:
                ...
                self.U[x, 1:-1, self.t_n+1] = self.U[x, 1:-1, self.t_n] \
                    + self.mu*(self.U[x+1, 1:-1, self.t_n]
                               - 2*self.h*self.conds[abs(x)+1](self.x)
